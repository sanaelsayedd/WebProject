...
if (isset($_GET['PurchaseTransactionID'])) {
    ...
    $query = "SELECT * FROM `purchase_transaction` WHERE `PurchaseTransactionID` = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query = "UPDATE `purchase_transaction` SET `UserID` = ?, `BookID` = ?, `Quantity` = ?, `TotalPrice` = ? WHERE `PurchaseTransactionID` = ?";
    $stmt = mysqli_prepare($connection, $update_query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "iiidi", $UserID, $BookID, $Quantity, $TotalPrice, $PurchaseTransactionID);

        if (mysqli_stmt_execute($stmt)) {
            header("Location: Purchase.php?status=success");
            exit();
        } else {
            echo "Error updating Purchase: " . mysqli_stmt_error($stmt);
        }
        mysqli_stmt_close($stmt);
    } else {
...
<head>
    ...
    <title>Edit PurchaseTransaction</title>
    <style>
...
<div class="dashboard-container">
    <div class="dashboard-content">
        ...
        <form action="EditPurchase.php?PurchaseTransactionID=<?php echo htmlspecialchars($PurchaseTransactionID); ?>" method="POST">
            <table>
                <tr>
                    <td>UserID:</td>
                    <td>
                ...
                <tr>
                    <td>TotalPrice:</td>
                    <td><input type="number" name="TotalPrice" value="<?php echo htmlspecialchars($TotalPrice); ?>" required></td>
                </tr>
            </table>
